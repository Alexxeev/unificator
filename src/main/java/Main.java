...
public class Main {
    ...
    public static void main(String[] args) {
        if (args.length != 3) {
            System.out.println(
                    "Provide the name of the unification" +
                    " algorithm and two terms enclosed" +
                    " in the double quotes");
            System.exit(1);
        }
        UnificationStrategy strategy =
                UnificationStrategyFactory.createInstance(args[0]);
        TermPair termPair = TermPair.fromStrings(args[1], args[2]);

        UnificationResult unificationResult = strategy
                .findUnifier(termPair);
